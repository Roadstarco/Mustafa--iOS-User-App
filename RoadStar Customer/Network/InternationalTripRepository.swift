//
//  InternationalTripRepository.swift
//  RoadStar Customer
//
...
class InternationalTripRepository{
    
    func sendRequestForIntTrip(request:InternationalRequest, attachemnts:[String], block: @escaping (InternationalRequestResponse?, Error?) -> Void) {
        let url = "https://myroadstar.org/api/user/create-trip"
        ...
        if attachemnts.count == 0
        {
            ...
            AF.request(url, method: .post, parameters: request, encoder: JSONParameterEncoder.default, headers: headers).validate(statusCode: 200..<600).validate(contentType: ["application/json"]).responseDecodable(of: InternationalRequestResponse.self){ (response) in
        ...
        else
        {
            ...
            .responseDecodable(of: InternationalRequestResponse.self){ (response) in
